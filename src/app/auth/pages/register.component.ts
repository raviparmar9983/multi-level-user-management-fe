...
@Component({
    ...
    <section class="auth-shell">
      <div class="auth-card">
        <div class="auth-copy">
          ...
          <p>We’ll send a verification link after account creation.</p>
        </div>

        <form class="auth-form" [formGroup]="form" (ngSubmit)="submit()">
          <app-form-input id="register-name" label="Name" autocomplete="name" placeholder="Your full name" [control]="form.controls.name" [errorMessages]="nameErrors" />
          <app-form-input id="register-email" label="Email" type="email" autocomplete="email" placeholder="you@example.com" [control]="form.controls.email" [errorMessages]="emailErrors" />
          <app-form-input id="register-password" label="Password" type="password" autocomplete="new-password" placeholder="Create a password" [control]="form.controls.password" [errorMessages]="passwordErrors" />
  ...
  styles: [`
    ...
    .auth-card { width: min(100%, 30rem); background: rgba(255,255,255,0.94); border: 1px solid rgba(148,163,184,0.2); border-radius: 1.5rem; padding: 2rem; box-shadow: 0 24px 50px rgba(15,23,42,0.08); }
    ...
    .primary-btn { border: 0; border-radius: 0.9rem; padding: 0.95rem 1.15rem; background: linear-gradient(135deg, #0f766e, #2563eb); color: #fff; font-size: 1rem; font-weight: 700; cursor: pointer; }
...
export class RegisterComponent {
  ...
  readonly nameErrors = { required: 'Name is required.' };
